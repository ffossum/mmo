...
1. [x] **Track connected players and their state on the server.**
   ...
   echoes them back. Add a data structure (e.g. a HashMap keyed by peer ID) that
   stores each connected player's current position, velocity, and yaw. Create
   and remove entries on connect/disconnect events. Deserialize incoming
   `PlayerIntent` messages into this state instead of echoing raw bytes. This is
   a prerequisite for everything below — the server needs to know who is
...
1. [ ] **Client-side prediction and server reconciliation.**
   ...
   To remove the choppiness from step 5: the client continues to simulate
   movement locally on every physics frame for instant responsiveness. Each
   `PlayerIntent` is tagged with a sequence number and buffered. When the server
   sends back an authoritative state tagged with the last processed sequence
   number, the client compares its predicted position at that sequence to the
   server's position. If they match (within a small tolerance), do nothing. If
   they diverge, snap to the server position and replay all buffered inputs that
   the server hasn't yet confirmed. This gives responsive controls while keeping
...
1. [ ] **Spawn and despawn remote player entities on the client.**
   ...
   (signaled by the server via a disconnect message or absence from snapshots
   for N ticks), remove their scene. Maintain a dictionary of
...
1. [ ] **Interpolation for remote players.**
   ...
   smoothly instead of teleporting between updates. Use linear interpolation for
   position and shortest-arc lerp for yaw.
...
1. [ ] **Basic chat.**
   ...
   client displays incoming chat messages in a scrollable UI panel. No channels,
   whispers, or history — just global broadcast chat.
